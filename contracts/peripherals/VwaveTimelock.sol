...
contract VwaveTimelock is IVwaveTimelock {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "VwaveTimelock: forbidden");
        _;
    }

    modifier onlyAdminOrHandler() {
        require(msg.sender == admin || isHandler[msg.sender], "VwaveTimelock: forbidden");
        _;
    }

    modifier onlyTokenManager() {
        require(msg.sender == tokenManager, "VwaveTimelock: forbidden");
        _;
    }

    modifier onlyRewardManager() {
        require(msg.sender == rewardManager, "VwaveTimelock: forbidden");
        _;
    }
    ...
    constructor(
        ...
        require(_buffer <= MAX_BUFFER, "VwaveTimelock: invalid _buffer");
        require(_longBuffer <= MAX_BUFFER, "VwaveTimelock: invalid _longBuffer");
        admin = _admin;
        buffer = _buffer;
    ...
    function setBuffer(uint256 _buffer) external onlyAdmin {
        require(_buffer <= MAX_BUFFER, "VwaveTimelock: invalid _buffer");
        require(_buffer > buffer, "VwaveTimelock: buffer cannot be decreased");
        buffer = _buffer;
    }

    function setMaxLeverage(address _vault, uint256 _maxLeverage) external onlyAdmin {
      require(_maxLeverage > MAX_LEVERAGE_VALIDATION, "VwaveTimelock: invalid _maxLeverage");
    ...
    function setFundingRate(address _vault, uint256 _fundingInterval, uint256 _fundingRateFactor, uint256 _stableFundingRateFactor) external onlyAdmin {
        require(_fundingRateFactor < MAX_FUNDING_RATE_FACTOR, "VwaveTimelock: invalid _fundingRateFactor");
        require(_stableFundingRateFactor < MAX_FUNDING_RATE_FACTOR, "VwaveTimelock: invalid _stableFundingRateFactor");
    ...
    function setFees(
        ...
        require(_taxBasisPoints < MAX_FEE_BASIS_POINTS, "VwaveTimelock: invalid _taxBasisPoints");
        require(_stableTaxBasisPoints < MAX_FEE_BASIS_POINTS, "VwaveTimelock: invalid _stableTaxBasisPoints");
        require(_mintBurnFeeBasisPoints < MAX_FEE_BASIS_POINTS, "VwaveTimelock: invalid _mintBurnFeeBasisPoints");
        require(_swapFeeBasisPoints < MAX_FEE_BASIS_POINTS, "VwaveTimelock: invalid _swapFeeBasisPoints");
        require(_stableSwapFeeBasisPoints < MAX_FEE_BASIS_POINTS, "VwaveTimelock: invalid _stableSwapFeeBasisPoints");
        require(_marginFeeBasisPoints < MAX_FEE_BASIS_POINTS, "VwaveTimelock: invalid _marginFeeBasisPoints");
        require(_liquidationFeeUsd < 10 * PRICE_PRECISION, "VwaveTimelock: invalid _liquidationFeeUsd");
    ...
    function setTokenConfig(
        ...
        require(_minProfitBps <= 500, "VwaveTimelock: invalid _minProfitBps");

        IVault vault = IVault(_vault);
        require(vault.whitelistedTokens(_token), "VwaveTimelock: token not yet whitelisted");
    ...
    function setInPrivateTransferMode(address _token, bool _inPrivateTransferMode) external onlyAdmin {
        if (excludedTokens[_token]) {
            ...
            require(_inPrivateTransferMode == false, "VwaveTimelock: invalid _inPrivateTransferMode");
        }
    ...
    function _mint(address _token, address _receiver, uint256 _amount) private {
        ...
        require(IERC20(_token).totalSupply() <= maxTokenSupply, "VwaveTimelock: maxTokenSupply exceeded");
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action] != 0, "VwaveTimelock: action not signalled");
        require(pendingActions[_action] < block.timestamp, "VwaveTimelock: action time not yet passed");
    }
