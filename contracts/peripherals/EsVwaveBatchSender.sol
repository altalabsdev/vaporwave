...
contract EsVwaveBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esVwave;

    constructor(address _esVwave) {
        admin = msg.sender;
        esVwave = _esVwave;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsVwaveBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esVwave).transferFrom(msg.sender, _accounts[i], _amounts[i]);
